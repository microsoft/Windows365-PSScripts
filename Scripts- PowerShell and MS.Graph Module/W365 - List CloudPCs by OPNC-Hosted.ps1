...
$OPNCs = Get-MgDeviceManagementVirtualEndpointOnPremisesConnection
$CloudPCs = Get-MgDeviceManagementVirtualEndpointCloudPC

foreach ($OPNC in $OPNCs) {
    Write-Host ""
    Write-Host $OPNC.DisplayName
    foreach ($CloudPC in $CloudPCs) {
        if ($CloudPC.OnPremisesConnectionName -eq $OPNC.DisplayName) {
            Write-Host $CloudPC.DisplayName
        }
    }
}

Write-Host ""
Write-Host "Microsoft Hosted Network"
foreach ($CloudPC in $CloudPCs) {
    if ($CloudPC.OnPremisesConnectionName -eq "") {
