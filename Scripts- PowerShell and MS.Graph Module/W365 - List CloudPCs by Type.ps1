...
Write-Output "Enteprise SKU CloudPCs" | Out-Host
foreach ($CloudPC in $CloudPCs) { if ($CloudPC.ServicePlanType -eq "enterprise") { Write-Output $CloudPC.Displayname | out-host } }

Write-Host " " | Out-Host

Write-Output "Business SKU CloudPCs" | Out-Host
foreach ($CloudPC in $CloudPCs) { if ($CloudPC.ServicePlanType -eq "business") { Write-Output $CloudPC.Displayname | out-host } }
