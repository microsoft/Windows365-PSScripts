...
$ProvPols = Get-MgDeviceManagementVirtualEndpointProvisioningPolicy

foreach ($ProvPol in $ProvPols) {
    write-host ""
    Write-Host $ProvPol.DisplayName
    foreach ($CloudPC in $CloudPCs) {
        if ($CloudPC.ProvisioningPolicyName -eq $ProvPol.DisplayName) { write-host $CloudPC.ManagedDeviceName }
